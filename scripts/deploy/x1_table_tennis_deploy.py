...
class X1RallyController:
    """Wraps X1TableTennisController with rally logic and closed-loop phase.

    Usage (50Hz loop):
        controller = X1RallyController(policy_path=...)
        while True:
    ...
    def __init__(self, policy_path: str = None, checkpoint_path: str = None,
        ...
        self.inner = X1TableTennisController(
    ...
    def _ball_incoming(self, ball_pos: np.ndarray, ball_vel: np.ndarray) -> bool:
        """Detect if a ball is coming toward the robot."""
        return (
            ball_vel[0] > 1.0 and          # Moving toward robot (positive x)
    ...
    def _correct_phase(self, ball_pos: np.ndarray, ball_vel: np.ndarray):
        ...
        Analogous to foot-contact phase reset in locomotion, but soft.
        """
        if ball_vel[0] < 0.5:
            return  # Ball not moving toward us, skip correction
