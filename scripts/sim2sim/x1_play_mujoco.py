...
def _fix_robot_base(root: ET.Element):
    ...
    # Rotate 180° around z-axis so robot faces the table (-x direction)
    robot_body.set("pos", f"{ROBOT_POS[0]} {ROBOT_POS[1]} {ROBOT_POS[2]}")
    robot_body.set("quat", "0 0 0 1")
...
def _add_ball(worldbody: ET.Element, ball_cfg: dict):
    ...
    # Restitution via negative solref: [-stiffness, -damping]
    # For COR ≈ 0.905 with 2.7g ball: high stiffness, minimal damping
...
def _replace_actuators(root: ET.Element):
    ...
    Match IsaacSim implicit actuator: tau = clamp(kp*(target-q) - kd*dq, -effort, +effort)
    """
    ...
    # Lift joint: high-stiffness position hold with moderate compliance
    # to absorb swing reaction forces (matching real robot's mechanical flexibility)
...
def _increase_joint_damping(root: ET.Element):
    """Lock non-controlled joints by constraining their range, and set
    damping/armature for stable arm tracking."""
...
def _weld_locked_joints(root: ET.Element):
    ...
    Keep lift joint (with moderate compliance to absorb swing reaction forces,
    matching real robot's mechanical flexibility).
    """
...
class URDFForwardKinematics:
    """Lightweight FK for X1 right arm chain: base_link → lift → yb_1..7 → paddle.

    Computes racket position from joint angles using URDF kinematic parameters,
    simulating what deployment code would do on the real robot (no simulator).
    """

    def __init__(self, robot_pos: np.ndarray):
        self.robot_pos = robot_pos.copy()
        # Kinematic chain from URDF: (parent→child joint origin xyz, rpy, axis, type)
        # base_link → link_lift (joint_lift, prismatic along z)
        # link_lift → link_yb_1 (joint_yb_1)
        # link_yb_1 → link_yb_2 (joint_yb_2)
        # ... → Link_yb_paddle (joint_yb_paddle, fixed)
        self.chain = [
    ...
    def compute(self, joint_lift: float, joint_angles: np.ndarray) -> np.ndarray:
        ...
        """
        # Start from robot base in world frame
        # Robot is rotated 180° about z-axis (facing the table, i.e. -x direction)
        # This matches both IsaacSim rot=(0,0,0,1) and MuJoCo quat="0 0 0 1"
        pos = self.robot_pos.copy()
        rot = self._rot_z(np.pi)  # 180° z-rotation
...
class X1Sim2SimRunner:
    ...
    def _get_joint_vel(self) -> np.ndarray:
        ...
        # Clamp to IsaacSim velocity_limit_sim: 8 rad/s for shoulder, 20 rad/s for wrist
    ...
    def _get_racket_pos(self) -> np.ndarray:
        ...
        # Compare with MuJoCo direct read for validation
    ...
    def _apply_targets(self, targets: np.ndarray):
        ...
        Emulates IsaacSim's velocity_limit_sim: when joint velocity exceeds
        the limit, the actuator output is scaled down, preventing overshoot.
        Without this, MuJoCo joints overshoot the reference and create
        out-of-distribution observations that confuse the policy.
        """
        ...
        for i, aid in enumerate(self.arm_actuator_ids):
            ...
            vlim = vel_limits[i]
            if qvel > vlim * 0.7:
                # Blend target toward current position to brake the joint
    ...
    def _reset_robot_to_ref(self):
        """Reset ALL robot joints to their initial positions (matching IsaacSim reset).

        In IsaacSim, robot resets to default_joint_pos (from ArticulationCfg.init_state).
        We must also reset non-arm joints (lift, left arm, head, wheels) to prevent drift.
        """
        # Reset arm joints to default
    ...
    def _run_step(self, step_count: int) -> dict:
        ...
        if self.args.no_policy:
            # Pure reference playback — no policy, just track ref_dof
        ...
        # Always print ball-racket distance for proximity analysis
        ...
        if self.args.no_policy:
            phase_speed = 1.0  # constant speed for reference playback
    ...
    def run_headless(self, num_episodes: int = 5, video_path: str = None):
        ...
        print(f"Running headless for {num_episodes} episodes...")
        print(f"  SIM_DT={SIM_DT}, STEP_DT={STEP_DT}, DECIMATION={DECIMATION}")
...
def main():
    ...
    parser.add_argument("--video", default="x1_sim2sim.mp4",
    ...
    if args.policy is None and args.checkpoint is None:
        ...
        default = os.path.join(
            ...
            "logs/rsl_rl/x1_tabletennis/2026-05-21_02-19-50/exported/policy.pt"
        )
