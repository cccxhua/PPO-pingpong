...
def ball_return_reward(
    ...
    crossing = (ball_pos[:, 0] < net_x) & (ball_vx < -0.5)
...
def ball_hit_toward_opponent(
    ...
    ball_vx = ball.data.root_lin_vel_w[:, 0]
    # Fix H: 平滑 shaping. 旧 clamp(-vx/3, 0, 1) 在 vx>0 区间硬归零, policy 学不会
    # "减速 → 反向" 的过渡. 新映射: 任何减速都有正梯度, 完全反向 (vx=-max_vel) 给 1.0.
    #   vx = +incoming_vx → 0   (零拦截)
    #   vx = 0             → incoming_vx / (incoming_vx + max_vel)
    #   vx = -max_vel      → 1.0 (硬挥反弹)
    return_vel = torch.clamp(
        (incoming_vx - ball_vx) / (incoming_vx + max_vel), 0.0, 1.0
    )
    return hit.float() * return_vel
...
def ball_speed_after_hit(
    ...
    speed_reward = torch.clamp((ball_speed - min_speed) / (max_speed - min_speed), min=0.0, max=1.0)
...
def racket_approach_ball_vel(
    env: ManagerBasedRLEnv, ball_name: str, racket_body_name: str, max_vel: float = 2.0
) -> torch.Tensor:
    ball: RigidObject = env.scene[ball_name]
    robot = env.scene["robot"]
    ...
    return torch.clamp(approach_vel / max_vel, min=-1.0, max=1.0)
...
def ball_velocity_match_ideal(
    ...
    ideal_vel = compute_ideal_ball_velocity(ball_pos_local, target_x, target_z)
...
def racket_swing_toward_ideal(
    ...
    """Dense reward: when ball is near, reward racket swinging in the ideal hit direction."""
    ball: RigidObject = env.scene[ball_name]
    robot = env.scene["robot"]
    ...
    racket_vel = _racket_world_vel(robot, racket_body_name)
    racket_speed = torch.norm(racket_vel, dim=-1, keepdim=True).clamp(min=1e-6)
    racket_dir = racket_vel / racket_speed

    alignment = torch.sum(racket_dir * ideal_dir, dim=-1).clamp(min=0.0)
    speed_scale = torch.clamp(racket_speed.squeeze(-1) / max_vel, max=1.0)

    gate = (dist < proximity_gate).float()
    return gate * alignment * speed_scale
...
def phase_ball_alignment_reward(
    ...
    t_arrive = torch.where(vx > 0.1, (robot_x - bx) / vx, torch.full_like(bx, 3.0)).clamp(0.0, 3.0)
...
def racket_at_predicted_hit(
    ...
    safe_vx = torch.where(vx > 0.1, vx, torch.ones_like(vx))
    t_arrive = (robot_x - bx) / safe_vx
    valid = (vx > 0.1) & (t_arrive > 0.0) & (t_arrive < 1.0)
