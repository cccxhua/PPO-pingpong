...
def ball_landed_on_own_table(
    ...
    """End episode when ball (after racket contact) reaches own table side at table height.

    Triggers when ball_was_hit=True AND ball z is within z_margin of table top AND on own side.
    Equivalent to "first landing point on own side" — ball_was_hit is False before paddle contact
    (so launch bounce on own table doesn't count), and True only after; once ball returns to
    table height on own side, this fires (and DoneTerm makes it one-shot, so multi-bounce is fine).
    Replaces previous z+vz heuristic which only had ~23ms detection window and often missed frames.
    """
    ...
    near_table = ball_pos[:, 2] < table_z + z_margin
    on_own_side = (ball_pos[:, 0] > table_x_min) & (ball_pos[:, 0] < table_x_max)
    in_bounds_y = ball_pos[:, 1].abs() < table_y_half

    return near_table & on_own_side & in_bounds_y & command.ball_was_hit
...
def ball_missed_paddle(
    ...
    ball: RigidObject = env.scene[ball_name]
    ball_pos = ball.data.root_pos_w[:, :3] - env.scene.env_origins
    past_robot = ball_pos[:, 0] > robot_x + margin
