...
class ActionsCfg:
    ...
    right_arm = mdp.ReferenceResidualJointActionCfg(
        ...
        command_name="motion",
        # Fix I 已 revert: 0.10 重导致 policy 整体回退 (接触率 -90%, error_joint_vel +16%).
        # 根因: action_std=0.02 已塌, 同 μ × 2 scale 直接破坏已学 calibration, 不是探索.
        # 退回 0.05 恢复 24000-iter "软挡" 基线 (接触有, 反向无), 后续从奖励角度破局.
...
class CommandsCfg:
    motion = mdp.UpperBodyMotionCommandCfg(
        ...
        motion_files=[
            ...
            os.path.join(FOREHAND_DATA_DIR, "forehand_right.npz"),
        ],
...
class RewardsCfg:
    ...
    ball_hit_speed = RewTerm(
        ...
        weight=3.00,  # Fix N: 1.50 → 3.00. 配合 direction 砍权 + min_speed 降阈, 让"用力" 成为主信号.
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[RACKET_BODY_NAME]),
            "ball_name": "ball",
            "proximity_threshold": 0.15,
            "min_speed": 0.1,  # Fix N: 1.0 → 0.1, 打破"软碰=0 reward"死区, 让低速接触也有梯度往上爬.
            "max_speed": 5.0,
        },
    )
    ball_hit_direction = RewTerm(
        func=mdp.ball_hit_toward_opponent,
        weight=0.50,  # Fix N: 2.00 → 0.50. 旧 weight 是"轻碰最优"的最大诱因 (单项贡献 ~1.9). 砍到 0.5 让 speed 成主导, direction 降为辅助.
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[RACKET_BODY_NAME]),
            "ball_name": "ball",
            "proximity_threshold": 0.15,
