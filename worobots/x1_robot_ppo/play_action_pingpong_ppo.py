...
PPO 乒乓球专用动作播放节点，50Hz 控制频率（匹配训练）。
只控制右臂 7 关节，无夹爪。

订阅话题:
  x1/sent_actions   (sensor_msgs/JointState)  — PPO 推理输出的关节目标位置
...
PUBLISH_HZ = 50.0  # PPO 训练频率 = 50Hz


class ActionPlayerPingPongPPO(Node):
    def __init__(self):
        super().__init__("action_player_pingpong_ppo")
        ...
        self.get_logger().info(
            f"ActionPlayerPingPongPPO started @ {PUBLISH_HZ} Hz. "
    ...
    def timer_callback(self) -> None:
        ...
        if current_time - self.last_log_time >= 2.0:
            ...
            self.get_logger().info(f"right arm cmd: {[f'{v:.4f}' for v in right_arm]}")
