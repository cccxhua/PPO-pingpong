...
class RightArmStateRecorderPPO(Node):
    def __init__(self):
        super().__init__("right_arm_state_recorder_ppo")
        ...
        # 数值微分用 (如果 /joint_states 不提供 velocity)
        ...
        self.get_logger().info(
            f"RightArmStateRecorderPPO started @ {PUBLISH_HZ} Hz. "
            "Publishing pos+vel to x1/recorded_joint_states_pingpong"
        )
    ...
    def timer_callback(self) -> None:
        ...
        msg.position = list(map(float, self.right_arm_pos))
        msg.velocity = list(map(float, self.right_arm_vel))
        msg.effort = [0.0] * 7
        ...
        if current_time - self.last_log_time >= 2.0:
            ...
            self.get_logger().info(
                f"pos: {[f'{v:.4f}' for v in msg.position]}"
            )
            self.get_logger().info(
                f"vel: {[f'{v:.4f}' for v in msg.velocity]}"
            )
